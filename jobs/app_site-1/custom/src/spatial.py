...
from src.sample_scalar import sample_scalar
...
class SpatialTransform(BasicTransform):
    ...
    def get_parameters(self, **data_dict) -> dict:
        ...
        if do_rotation:
            x_angles=((-120 / 360 * 2. * np.pi, -60. / 360 * 2. * np.pi),(-30 / 360 * 2. * np.pi, 30. / 360 * 2. * np.pi))
            angles_changed=[ x_angles,self.rotation,self.rotation]
