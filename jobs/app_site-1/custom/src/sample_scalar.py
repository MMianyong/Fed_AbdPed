...
def sample_scalar(scalar_type: RandomScalar, *args, **kwargs):
   if isinstance(scalar_type, (int, float)):
       ...
       if isinstance(scalar_type[0], (list, tuple)):
           range1=np.random.randint(len(scalar_type))
           scale_type_set= scalar_type[range1]
           return np.random.uniform(*scale_type_set)
