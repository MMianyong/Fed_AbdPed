...
"""
Modify the spatialtransform in the data augmentation part of nnuent to make the rotation can be -90 to 90
...
class SpatialTransform(BasicTransform):
    ...
    def get_parameters(self, **data_dict) -> dict:
        ...
        if do_rotation:
            #Modify here for the -90 to 90
            angles_changed=[ ((-90 / 360 * 2. * np.pi, 90. / 360 * 2. * np.pi)),self.rotation,self.rotation]
            #angles = [sample_scalar(self.rotation, image=data_dict['image'], dim=i) for i in range(0, 3)]
            angles = [sample_scalar(angles_changed[i], image=data_dict['image'], dim=i) for i in range(dim)]
        else:
            angles = [0] * dim
        if do_scale:
